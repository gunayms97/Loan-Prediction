...
boxplot.stats(loan$LoanAmount)

upper.whisker <- boxplot.stats(loan$LoanAmount)$stats[5] #255
outlier.filter <- loan$LoanAmount < upper.whisker

loan[outlier.filter,]


l.equation <- "LoanAmount~Gender + Married + Dependents + Education + Self_Employed + ApplicantIncome + Loan_Amount_Term +Credit_History +Property_Area " 
l.model <- lm(formula = l.equation, data = loan[outlier.filter,])

l.row <-loan[is.na(loan$LoanAmount),c("Gender","Married",'Dependents','Education','Self_Employed',"ApplicantIncome","Loan_Amount_Term",'Credit_History','Property_Area')]

l.pred<-predict(l.model, newdata = l.row )

loan[is.na(loan$LoanAmount),"LoanAmount"] <- l.pred
...
write.csv(output.df, file = "av_loan2.csv", row.names = FALSE)
